...
export function GiftsPage() {
  ...
  const fetchGifts = useCallback(async () => {
    if (!telegramUser?.id) {
      console.log("[GiftsPage] No telegramUser.id available")
      return
    }

    setIsLoading(true)
    try {
      // Normalize telegramId to string
      const telegramIdStr = String(telegramUser.id)
      console.log(`[GiftsPage] Fetching gifts for telegramId: ${telegramIdStr} (original: ${telegramUser.id}, type: ${typeof telegramUser.id})`)
      
      // First, check debug endpoint to see all gifts
      try {
        const debugRes = await fetch(`/api/debug/gifts?userId=${telegramIdStr}`)
        const debugData = await debugRes.json()
        console.log(`[GiftsPage] Debug endpoint response:`, debugData)
      } catch (debugError) {
        console.warn("[GiftsPage] Debug endpoint failed:", debugError)
      }
      
      const res = await fetch(`/api/user/gifts?telegramId=${telegramIdStr}`)
      const data = await res.json()

      console.log(`[GiftsPage] API response:`, data)
      console.log(`[GiftsPage] Debug info:`, data.debug)
      console.log(`[GiftsPage] Gifts array:`, data.gifts)
      console.log(`[GiftsPage] Gifts length:`, data.gifts?.length)

      if (data.success) {
        const giftsArray = Array.isArray(data.gifts) ? data.gifts : []
        setGifts(giftsArray)
        setTotalCount(data.totalCount || 0)
        console.log(`[GiftsPage] Loaded ${giftsArray.length} gifts, setting state...`)
        
        if (giftsArray.length === 0) {
          console.warn(`[GiftsPage] ⚠️ No gifts found for user ${telegramIdStr}. This might be due to Vercel serverless instances - data may be lost between requests.`)
          console.warn(`[GiftsPage] ⚠️ Check /api/debug/gifts?userId=${telegramIdStr} to see if gifts exist in the current instance.`)
        }
      } else {
        console.error("[GiftsPage] API returned error:", data.error)
      }
    } catch (error) {
      console.error("[GiftsPage] Error fetching gifts:", error)
    } finally {
      setIsLoading(false)
    }
  }, [telegramUser?.id])
